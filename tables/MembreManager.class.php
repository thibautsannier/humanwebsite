...
class MembreManager{
		...
		public static function creer($m){
			...
			$res = DB::get_instance()->prepare($sql); // Paterne singleton qui créé pourla premère fois une instance de PDO, et la réutilise ensuite tout le temps (ne la recréé donc pas)
		...
		{
			$stmt=DB::get_instance()->prepare("SELECT user_id, first_name, last_name FROM users ORDER BY last_name LIMIT 10");
			$stmt->execute();
